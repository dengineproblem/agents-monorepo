...
2. Прочитай .claude/ads-agent/config/creatives/{account_name}.md (если есть)
   → Список готовых креативов для ротации
```
...
Пожиратели — объявления, которые тратят бюджет без результата.
...
| **HIGH** | Zero leads при spend ≥ 2× target |
| **HIGH** | CPL > 2× target И spend_share ≥ 50% |
| **MEDIUM** | CPL > 1.5× target при spend_share ≥ 50% |

### Логика определения

Если в ad set ≥2 объявлений:
1. Посчитай общие затраты: totalSpend = sum(ads[].spend)
2. Найди топ-спендера: topAd = max(ads[].spend)
3. Если topAd.spend ≥ 50% от totalSpend:
   - Посчитай его CPL
   - Если CPL > target × 1.3 → это пожиратель

### Что проверить перед паузой объявления

1. Посчитай сколько ВСЕГО активных объявлений в этом adset
2. Посчитай сколько из них — пожиратели
3. Определи: останутся ли объявления после паузы всех пожирателей?

**ВАЖНО:**
- Если после паузы останутся объявления → паузим пожирателя
- Если adset останется ПУСТЫМ → решай что делать с АДСЕТОМ целиком (пауза или оставить)
...
**Если снижаешь** бюджет adset с $50 до $25 (освобождается $25):
1. ЕСЛИ есть креативы → создай новый adset с бюджетом $25
2. ЕСЛИ креативов нет → увеличь бюджет других adsets (с HS ≥ good)

**Если паузишь** adset с бюджетом $50:
1. ЕСЛИ есть креативы → создай новый adset
2. ЕСЛИ креативов нет → перераспредели на другие adsets
...
### Создание новых ad sets

- Бюджет: 1000-2000 центов ($10-20) — НЕ БОЛЬШЕ $20
- За один запуск по направлению: 1-3 новых ad sets
- Если freed_budget > $35 → создай 3 adset по $10-15 каждый
