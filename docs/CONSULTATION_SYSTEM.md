...
#### `blocked_slots` - Заблокированные слоты (перерывы)
...
Эта таблица используется для одноразовых блокировок слотов (перерывы, обеды) на конкретную дату.
В отличие от `working_schedules`, это разовые блокировки, а не регулярные.
...
### Блокировка слотов (перерывы)

| Метод | Путь | Описание |
|-------|------|----------|
| GET | `/blocked-slots` | Список блокировок (по дате/консультанту) |
| POST | `/blocked-slots` | Создать блокировку |
| DELETE | `/blocked-slots/:id` | Удалить блокировку |

**Параметры GET `/blocked-slots`:**

| Параметр | Тип | Описание |
|----------|-----|----------|
| `date` | string | Конкретная дата (YYYY-MM-DD) |
| `consultant_id` | UUID | ID консультанта |
| `start_date` | string | Начало диапазона |
| `end_date` | string | Конец диапазона |

**Тело POST `/blocked-slots`:**
...
**UI в календаре:**
- При наведении на пустой слот появляется кнопка ☕ для блокировки
- Заблокированные слоты отображаются жёлтым цветом с причиной
- Клик на заблокированный слот позволяет разблокировать его
