...
| Tool | Описание |
|------|----------|
| `get_available_consultation_slots` | Получить доступные слоты для записи |
...
Бот: [вызывает get_available_consultation_slots]
     Вот доступные слоты:
     1. Сегодня в 14:00 — Анатолий
     2. Завтра в 10:00 — Анатолий
     3. 3 января (пятница) в 11:00 — Мария
...
Бот: [вызывает book_consultation с параметрами]
     {
       "consultant_id": "4d71d2",  // из системного промпта
       "date": "2025-01-02",       // из слота
       "start_time": "10:00"       // из слота
     }

     Отлично! Вы записаны на консультацию:
     📅 Дата: 2 января 2025
     ⏰ Время: 10:00
     👨‍💼 Консультант: Анатолий
