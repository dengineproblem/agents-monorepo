...
2. **Фильтрация** — обрабатываем только сообщения с `referral` (от рекламы)
3. **Поиск номера** — по `waba_phone_id` или `phone_number`
4. **Resolve creative** — определение креатива и направления по `source_id`
5. **Upsert lead** — создание или обновление лида
6. **Dialog analysis** — трекинг для CAPI
