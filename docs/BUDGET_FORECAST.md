...
### 2025-12-26
- **ИСПРАВЛЕНО:** Summary metrics (Затраты/неделю, Результаты/неделю, Средний CPR) теперь считаются по ВСЕМ объявлениям кампании, а не только по eligible
  - Ранее: `eligibleAds.reduce()` - считались только объявления с достаточной историей
  - Теперь: `adForecasts.reduce()` - считаются все объявления кампании
  - Прогнозы по-прежнему строятся только для eligible объявлений
- **Файл:** `services/budgetForecaster.ts`

### 2025-12-25
- **ИСПРАВЛЕНО:** Week 2 прогноз для scaling сценариев - убран slope
  - Slope применяется только к baseline прогнозам
  - Для scaling используется только эластичность k
- **ОБНОВЛЕНО:** Таблица показывает CPR для всех сценариев (+20%, +50%, +100%) одновременно
- **ОБНОВЛЕНО:** Русификация UI (Spend→Затраты, Results→Результаты)
