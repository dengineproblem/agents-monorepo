...
**API:**
- `GET /admin/ads/cpl-analysis?period=7d` — CPL анализ по направлениям (из Facebook API)
- `GET /admin/ads/creatives?period=7d` — список креативов (из creative_metrics_history)
- `GET /admin/ads/users-summary?period=7d` — ROI сводка по пользователям

**Вкладки:**

### 5.1 CPL Анализ
Сравнение планового и фактического CPL по направлениям.

| Метрика | Описание |
|---------|----------|
| Плановый CPL | Из настроек направления (target_cpl_cents) |
| Фактический CPL | spend / leads |
| Отклонение | ((actual - planned) / planned) * 100% |
| Статус | 🟢 в норме (<0%), 🟡 выше (0-20%), 🔴 критично (>20%) |

**Сортировка:** По отклонению или по расходу.

### 5.2 Креативы
Таблица креативов с метриками из `creative_metrics_history`:
- Название + пользователь
- Направление
- Расход
- Лиды
- CPL (фактический)
- План CPL (из направления)
- Отклонение %

**Оптимизация:** Метрики загружаются из `creative_metrics_history` вместо прямых запросов к Facebook API (быстрее ~50x).

### 5.3 ROI по пользователям
Сводка по рекламным расходам и доходам:
- Пользователь
- Расход (USD)
- Доход (KZT из leads.sale_amount)
- Лиды
- CPL
...
└── 087_fix_registration_trigger.sql # Фикс триггера регистрации
```

---

## Исправленные баги (сессия 2024-12-09)

### 1. Краш AdminLeads при выборе фильтра
**Причина:** Radix UI Select не допускает пустое значение `value=""` в SelectItem.
**Исправление:**
- Заменено `useState('')` → `useState('all')`
- Заменено `<SelectItem value="">` → `<SelectItem value="all">`
- Добавлена проверка `userFilter !== 'all'` перед отправкой в API

### 2. Краш AdminUsers при клике на профиль
**Причина:** Навигация на несуществующий роут `/admin/users/:id`.
**Исправление:** Изменена навигация на `/admin/chats/:userId` (переход в чаты пользователя).

### 3. Null-pointer ошибки в AdminAds
**Причина:** Вызов `.toFixed()` на потенциально undefined значениях.
**Исправление:** Добавлен null-coalescing: `(value ?? 0).toFixed(1)`
