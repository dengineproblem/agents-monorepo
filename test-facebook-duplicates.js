// Тест для проверки дубликатов от Facebook API
const https = require('https');

const AD_ACCOUNT_ID = 'act_120236462634870345';
...
const url = `https://graph.facebook.com/v21.0/${AD_ACCOUNT_ID}/insights?fields=campaign_name,campaign_id,spend,actions,cpm,ctr,impressions,frequency&date_preset=yesterday&level=campaign&action_breakdowns=action_type&limit=500&access_token=${ACCESS_TOKEN}`;
      ...
      campaignGroups.forEach((rows, campaign_id) => {
        ...
        if (rows.length > 1) {
          ...
          rows.slice(1).forEach((row, idx) => {
            ...
            console.warn(`      - Строка #${idx + 2}: spend=${skippedSpend}, leads=${skippedLeads.leads}`);
          });
          console.log('');
        }
