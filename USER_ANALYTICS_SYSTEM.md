...
- ✅ **Расширение уведомлений:**
  - ✅ `INACTIVE_3D` теперь отправляется и в Telegram (ранее только in_app)
  - ✅ Добавлен шаблон `ONBOARDING_FB_PENDING` для застрявших на подключении FB
  - ✅ Удалён шаблон `ACHIEVEMENT_5_CREATIVES` (не нужен)
  - ✅ Добавлен `HIGH_CPL_ALERT` — уведомление если CPL > планового 3 дня подряд
  - ✅ Добавлен `WEEKLY_REPORT` — еженедельный отчёт (понедельник 11:00 Алматы)
- ✅ **Еженедельный отчёт включает:**
  - Общие траты за неделю
  - Количество лидов
  - Процент качества лидов (квалифицированных)
  - Средний CPL
  - Доход и ROI (если есть данные о продажах)
