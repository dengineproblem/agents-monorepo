...
=== АКТУАЛЬНЫЙ КОНТЕКСТ (ОБЯЗАТЕЛЬНО ИСПОЛЬЗУЙ, ЕСЛИ РЕЛЕВАНТЕН) ===

${formatContextsForPrompt(activeContexts)}

ПРАВИЛА ИСПОЛЬЗОВАНИЯ КОНТЕКСТА:
1. Выбери НЕ БОЛЕЕ ОДНОГО контекста, который лучше всего подходит этому лиду
2. Используй контекст естественно, без навязчивости
3. КРИТИЧЕСКИ ВАЖНО: Если используешь контекст - включай ВСЕ конкретные детали (ссылки, цифры, даты, контакты) ДОСЛОВНО из контекста
4. НЕ пересказывай общими фразами типа "много полезной информации" - давай КОНКРЕТИКУ
5. Если в контексте есть ссылка - ОБЯЗАТЕЛЬНО включи её в сообщение
...
6. ОБРАЩЕНИЕ ПО ИМЕНИ:
   - Если имя клиента УКАЗАНО - используй его в приветствии естественно.
   - Если имя НЕ УКАЗАНО - используй общие фразы БЕЗ обращения по имени (например: "Здравствуйте!", "Добрый день!", "Напоминаю...").
   - НЕ используй заглушки типа "Клиент", "Уважаемый клиент" и т.п.
...
9. КРИТИЧНО - ПРОВЕРКА ДАТ: Внимательно изучи историю переписки. Если клиент упоминал консультацию/встречу на конкретную дату, ОБЯЗАТЕЛЬНО сверь её с текущей датой (${currentDateStr}). НЕ напоминай о встречах, которые УЖЕ ПРОШЛИ. Если встреча была в прошлом - фокусируйся на следующих шагах (например, "Как прошла консультация?"), а НЕ на напоминании о ней.
10. КРИТИЧНО - КОНКРЕТИКА ИЗ КОНТЕКСТА: Если используешь актуальный контекст - включай ВСЕ конкретные детали (ссылки, номера, цифры, даты) ДОСЛОВНО. НЕ пиши "много полезной информации" - давай КОНКРЕТИКУ из контекста.
11. ФОРМАТИРОВАНИЕ ДЛЯ ЧИТАЕМОСТИ: Разбивай текст на смысловые блоки для лучшей читаемости в WhatsApp. Каждые примерно 100 символов группируй предложения по смыслу и разделяй пустой строкой. Это делает сообщение более структурированным и удобным для чтения.
