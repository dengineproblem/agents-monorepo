...
Микросервис для автоматической простановки ярлыков квалифицированным лидам в WhatsApp Business через [whatsapp-web.js](https://github.com/nicholasrobinson/whatsapp-web.js), а также для восстановления пропущенных сообщений и AI-квалификации лидов.
...
Анализирует **все** диалоги за последние 24 часа через GPT-4o-mini и обновляет `is_qualified` в таблице `leads`. Квалификация может меняться в обе стороны (false→true, true→false) по мере развития диалога.
...
Шаг 1 — Квалификация (qualificationSync):
  ...
  2. Для каждого аккаунта:
     a. Получить dialog_analysis за последние 24 часа (incoming_count > 0)
...
## Процесс ежечасного крона (missedMessages)
...
2. Для каждого аккаунта:
   ...
   c. Для каждого чата с входящими:
      - Тип 1 (полный пропуск): нет записей в Evolution DB → push все входящие в chatbot
      - Тип 2 (пропуск в диалоге): меньше входящих в Evolution чем в wwebjs → push разницу
   d. Уничтожить сессию
```
