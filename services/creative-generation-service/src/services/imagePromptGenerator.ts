...
2) Safe-zone для формата 9:16 (Stories/Reels):
   - верхняя часть экрана (примерно 10%) занята интерфейсом (ник, аватар, прогресс-бар),
   - НИЖНЯЯ ТРЕТЬ ЭКРАНА (33%) ПОЛНОСТЬЮ ЗАКРЫТА интерфейсом Instagram: кнопка CTA ("Чат в WhatsApp"), описание поста, лайки, комментарии, кнопки отправки.

   КРИТИЧНО ДЛЯ 9:16: НИЖНЯЯ ТРЕТЬ ИЗОБРАЖЕНИЯ ДОЛЖНА БЫТЬ ПУСТОЙ ИЛИ СОДЕРЖАТЬ ТОЛЬКО ФОН!
   - НЕ размещай там текст, важные объекты, лица, кнопки или любые значимые элементы — они будут полностью закрыты UI Instagram.
   - Весь важный контент должен располагаться в ВЕРХНИХ ДВУХ ТРЕТЯХ изображения.

   Примечание: Это изображение будет автоматически адаптировано под формат 4:5 для Feed-плейсментов.
   В 4:5 весь текст будет виден, поэтому не нужно специально оставлять большие пустые зоны для 4:5.
   Основная задача — сделать хороший креатив для 9:16, где нижняя треть пуста.

Твоя задача — разместить ВАЖНЫЙ ТЕКСТ и главные визуальные акценты ТОЛЬКО в верхних 2/3 кадра:
   - не использовать нижнюю треть для важных надписей или объектов,
   - не прижимать текст к краям.

В финальном промпте обязательно укажи:
   - что весь текст и ключевые элементы должны находиться в ВЕРХНИХ ДВУХ ТРЕТЯХ кадра,
   - что НИЖНЯЯ ТРЕТЬ должна быть ПУСТОЙ (только фон, без текста и важных объектов),
...
4) РАСПОЛОЖЕНИЕ ТЕКСТА (УЧИТЫВАЯ SAFE-ZONE):
   ВАЖНО: ВЕСЬ ТЕКСТ ДОЛЖЕН РАСПОЛАГАТЬСЯ В ВЕРХНИХ 2/3 ИЗОБРАЖЕНИЯ!
   Нижняя треть полностью закрыта UI Instagram и должна быть пустой.

   - указать расположение ТОЛЬКО ТЕХ текстовых элементов, которые ПЕРЕДАНЫ во входных данных:
     • Заголовок (если указан) — крупный текст в верхней части (10-30% от верха);
     • Буллеты (если указаны) — 3 пункта в центральной части (30-55% от верха);
     • Текст выгоды (если указан) — ближе к середине, но НЕ НИЖЕ 60% от верха.
   ...
   - НИЖНЯЯ ТРЕТЬ (от 67% до 100%) — ТОЛЬКО ФОН, БЕЗ ТЕКСТА!
