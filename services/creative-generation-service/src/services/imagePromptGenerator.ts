...
6) freestyle
- Назначение:
  • этот стиль предоставляет ПОЛНУЮ СВОБОДУ пользователю;
  • НЕТ предустановленных правил по визуальному стилю, фону, персонажам, цветам или типу изображения;
  • пользователь САМ задаёт стиль через отдельный промпт.
- Когда использовать:
  • когда ни один из готовых стилей (modern_performance, live_ugc, visual_hook, premium_minimal, product_hero) не подходит;
  • для нестандартных креативов и экспериментов;
  • когда пользователь хочет полностью контролировать визуальную подачу.
- Что делать:
  • НЕ добавляй никакие стилевые ограничения или рекомендации от себя;
  • НЕ навязывай тип визуала (фото, иллюстрация, 3D, минимализм и т.п.);
  • НЕ задавай палитру, атмосферу или характер изображения;
  • ПОЛНОСТЬЮ полагайся на PROMPT4 и входные данные от пользователя.
- Базовые технические требования (сохраняются):
  • формат 4:5, 1080×1350;
  • текст размещается в центральной части (если передан);
  • safe-zone соблюдается;
  • единый фон на всё изображение без видимых границ.
- Текст и шрифты:
  • если не указаны особые требования — современный читаемый sans-serif;
  • текст должен быть контрастным и читаемым на любом фоне.
...
- При смене style_id общий вид креатива должен заметно меняться:
  ...
  • freestyle — ПОЛНОСТЬЮ определяется пользователем через PROMPT4, без стилевых ограничений.
