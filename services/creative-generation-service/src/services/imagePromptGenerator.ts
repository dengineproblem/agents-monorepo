...
4) РАСПОЛОЖЕНИЕ ТЕКСТА (УЧИТЫВАЯ SAFE-ZONE):
   - указать расположение ТОЛЬКО ТЕХ текстовых элементов, которые ПЕРЕДАНЫ во входных данных:
     • OFFER (если указан) — крупный заголовок в верхней части центральной зоны, ниже интерфейса;
     • BULLETS (если указаны) — 3 пункта в отдельном блоке/карточке в центре;
     • PROFITS (если указан) — строка выгоды ближе к нижней части центральной зоны, но не у самого низа;
     • CTA (если указан) — яркая кнопка над нижней опасной зоной, внутри безопасной области.
...
export async function generateImagePrompt(
  ...
  if (cta && cta.trim()) {
    textParts.push(`- CTA (текст на кнопке): "${cta}"`);
  }
