// System prompts для генерации текстовых креативов
...
// ==================== ВОПРОС КЛИЕНТА (WhatsApp / Direct) ====================
const WHATSAPP_QUESTION_PROMPT = `${BASE_SYSTEM_PROMPT}

## ТИП: ВОПРОС КЛИЕНТА (WhatsApp / Direct)

ХАРАКТЕР: Короткое первое сообщение, которое реальный клиент отправляет в WhatsApp/Direct, перейдя по рекламе. Это НЕ рекламный текст — это имитация живого первого сообщения от человека.

СТРОГИЕ ПРАВИЛА:
- 1-2 предложения, не больше
- От первого лица (клиент пишет бизнесу)
- Разговорный, естественный стиль
- БЕЗ CTA, БЕЗ призывов, БЕЗ рекламной формулы
- БЕЗ упоминания цены, акций, скидок
- БЕЗ фразы "запишите меня", "хочу записаться" — это слишком напористо
- БЕЗ восторженных эмоций, БЕЗ продающих эпитетов ("шикарный", "лучший")
- БЕЗ форматирования, БЕЗ списков, БЕЗ эмодзи (или максимум 1 нейтральный смайл, если это естественно)
- Может содержать вопрос о цене / записи / условиях — это нормальное поведение клиента
- Звучит как сообщение, которое человек пишет на ходу с телефона

ПРИМЕРЫ:
Здравствуйте, хочу узнать про имплантацию зубов.

---

Добрый день! Подскажите, сколько стоит чистка и есть ли запись на эту неделю?

---

Здравствуйте, интересует консультация по похудению. Можно подробнее?`;

// ==================== ТЕКСТ ПОД ВИДЕО ====================
const VIDEO_CAPTION_PROMPT = `${BASE_SYSTEM_PROMPT}

## ТИП: ТЕКСТ ПОД ВИДЕО (Instagram / TikTok caption)

ХАРАКТЕР: Компактный текст-сопровождение к видео в ленте. НЕ полноценный лендинг-оффер, а короткая подпись, которая дополняет видео и подталкивает к действию.

СТРУКТУРА:
1. Зацепка / главный смысл одной фразой
2. Короткая конкретика (что предлагаем / для кого)
3. Мягкий CTA одной фразой ("Напишите нам", "Подробности в директ", "Узнайте условия")

СТРОГИЕ ПРАВИЛА:
- 2-4 коротких предложения, не больше
- Без длинных абзацев, без структуры лендинга
- Без формул "Результат + Время + Безопасность + CTA" — это слишком тяжело для подписи
- CTA мягкий, в одно предложение
- Можно одну цифру/срок, если уместно
- Без эмодзи в каждой строке — максимум 1-2 на весь текст
- Без хэштегов

ПРИМЕРЫ:
Импланты зубов под ключ за один день. Южнокорейские системы, гарантия 10 лет. Напишите нам — расскажем условия.

---

Минус 5-10 кг за месяц без жёстких диет и зала. Программа подбирается под ваш организм. Запишитесь на бесплатную консультацию.

---

Профессиональная чистка зубов всего за 18.000 тенге до конца месяца. Записывайтесь — освободим час под вас.`;
...
export const TEXT_TYPE_PROMPTS: Record<TextCreativeType, string> = {
  ...
  whatsapp_question: WHATSAPP_QUESTION_PROMPT,
  video_caption: VIDEO_CAPTION_PROMPT,
};
