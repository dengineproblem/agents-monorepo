...
const SYSTEM_PROMPT = (clientPrompt) => [
  ...
  '- slightly_bad: снижай daily_budget на −20..−50%; лечи креатив (PauseAd «пожирателя»); ВМЕСТО дублирования проверь креативы: unused_creatives (новый контент) или ready_creatives (ротация).',
  '- bad: ВЫБОР между двумя вариантами:',
  '  • Вариант A (снижение на -50%): если CPL превышает целевой в 2-3 раза, но есть показы/лиды → снижай бюджет на −50%, освободившиеся деньги переноси на новую кампанию (unused_creatives приоритетнее, иначе ready_creatives)',
  '  • Вариант B (полная пауза): если CPL превышает в >3 раза ИЛИ spend есть но лидов нет → PauseAdset, освободившийся бюджет (100%) переноси на новую кампанию (unused_creatives приоритетнее, иначе ready_creatives)',
