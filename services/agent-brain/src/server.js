...
const SYSTEM_PROMPT = (clientPrompt) => [
  ...
  '  • unused_creatives: список креативов которые готовы к использованию но НЕ ИСПОЛЬЗУЮТСЯ в активных ads сейчас (с рекомендуемым objective)',
  ...
  '3. **Неиспользованные креативы (КЛЮЧЕВОЕ!)**: если в unused_creatives есть креативы — создай НОВУЮ кампанию с НЕСКОЛЬКИМИ креативами в ОДНОМ adset ВМЕСТО дублирования. Приоритет: 3 креатива → 2 креатива → 1 креатив. Это дает шанс текущим кампаниям продолжить обучение + тестирует новый контент без конкуренции. ВСЕ креативы должны быть в ОДНОМ adset (разные ads), НЕ создавай отдельные кампании для каждого!',
  ...
  '- slightly_bad: снижай daily_budget на −20..−50%; лечи креатив (PauseAd «пожирателя»); ВМЕСТО дублирования проверь unused_creatives и создай новую кампанию (см. ниже).',
  ...
  '- Назначение: смена аудитории на LAL 3% IG Engagers 365d (не тестирует новый контент)',
  '- Условия:',
  ...
  '',
  'ПРИМЕР 4 (фолбэк на LAL дубль если unused_creatives пусто)',
  'Example JSON:\n{\n  "planNote": "HS bad, unused_creatives=[]. Фолбэк: LAL дубль т.к. нет новых креативов.",\n  "actions": [\n    { "type": "GetCampaignStatus", "params": { "campaign_id": "<CAMP_ID>" } },\n    { "type": "Audience.DuplicateAdSetWithAudience", "params": { "source_adset_id": "<ADSET_ID>", "audience_id": "test_lal_ig_365d", "daily_budget": 1000, "name_suffix": "LAL3" } }\n  ],\n  "reportText": "📊 Отчет\\n\\nТекущая кампания показывает плохие результаты. Новых креативов для теста нет, поэтому создаем дубль с новой аудиторией (LAL 3% Instagram Engagers). Бюджет $10/день."\n}',
  '',
  'Тул: SendActions',
  `- POST ${AGENT_URL}`,
