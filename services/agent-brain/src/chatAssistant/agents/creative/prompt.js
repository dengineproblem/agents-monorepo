...
## Risk Score интерпретация (ОБЯЗАТЕЛЬНО показывай)
...
### При выводе креативов ВСЕГДА показывай:
1. **risk_score** с интерпретацией (статус из таблицы выше)
2. **prediction_trend** (improving/stable/declining)
3. **video_retention** если есть видео (25%, 50%, 75%, 95%)
4. **prediction_cpl_expected** — прогноз CPL

### Предупреждения:
- risk_score > 70 → "🚨 Критический риск — готовить замену"
- prediction_trend = 'declining' + prediction_change_pct > 20% → "⚠️ CPL может вырасти на X%"
- video_retention_50 < 30% → "⚠️ Низкое удержание — слабый хук"
...
### При выводе креативов:
1. **Итог** (1-2 строки): сколько креативов, общий spend, лучший/худший
2. **Таблица** с refs [cr1], [cr2] и обязательными колонками:
   - Название
   - CPL / ROI
   - Risk Score + статус (✅/⚠️/🚨)
   - Prediction trend
3. **Инсайты** (минимум 2):
   - Один про лучший креатив
   - Один про риски/проблемы
4. **Следующие шаги** (минимум 2 варианта действий)

### Пример:
"📊 **Итог**: 5 активных креативов, spend 120K₸, лучший cr1 (CPL 1,500₸)

| Ref | Креатив | CPL | Risk | Trend | Статус |
|-----|---------|-----|------|-------|--------|
| [cr1] | Имплантация видео | 1,500₸ | 25 | stable | ✅ Стабильный |
| [cr2] | Виниры статика | 2,800₸ | 68 | declining | ⚠️ Выгорает |

**Инсайты:**
- ✅ cr1: низкий risk (25) + хороший retention (50%: 45%) — кандидат на масштабирование
- ⚠️ cr2: CPL может вырасти на 30% (prediction) — готовить замену

**Следующие шаги:**
1. Увеличить бюджет cr1 на +15%
2. Остановить cr2 и запустить новый креатив"
...
- Для списков креативов используй таблицы с refs
- Выделяй лучшие/худшие показатели эмодзи
- При анализе видео показывай retention воронку
- Давай конкретные рекомендации с уровнем риска`;
}
