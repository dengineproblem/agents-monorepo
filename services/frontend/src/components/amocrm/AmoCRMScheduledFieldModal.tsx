...
import { Check, X, Calendar, AlertCircle, Loader2, List, Plus, Trash2, CheckSquare } from 'lucide-react';
import {
  getLeadCustomFields,
  getScheduledFields,
  setScheduledFields,
...
interface AmoCRMScheduledFieldModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const AmoCRMScheduledFieldModal: React.FC<AmoCRMScheduledFieldModalProps> = ({
  isOpen,
  onClose,
  userAccountId,
  onSave,
  ...
  const loadData = async () => {
    try {
      ...
      try {
        const settingsResponse = await getScheduledFields(userAccountId);
  ...
  const handleSave = async () => {
    try {
      ...
      await setScheduledFields(userAccountId, fieldsToSave);

      if (fieldsToSave.length > 0) {
        const fieldNames = fieldsToSave.map(f => f.field_name).join(', ');
        setSuccessMessage(
          fieldsToSave.length === 1
            ? `Поле "${fieldNames}" установлено для определения записи`
            : `Поля (${fieldsToSave.length}) установлены: ${fieldNames}`
        );
      } else {
        setSuccessMessage('Определение записи по полям AmoCRM отключено');
      }
      ...
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Настройка определения записи (Schedule)
          </DialogTitle>
          <DialogDescription>
            Выберите до {MAX_FIELDS} полей в карточке AmoCRM для определения записи на консультацию.
            Событие Schedule будет отправлено в Facebook, если хотя бы одно поле установлено.
          </DialogDescription>
        </DialogHeader>
                ...
                return (
                  <div key={index} className="space-y-2 p-3 border rounded-lg bg-gray-50 dark:bg-gray-800 dark:border-gray-700">
                    ...
                    {needsEnumSelection(currentField) && (
                      <div className="mt-2">
                        <label className="text-xs text-muted-foreground">Значение = записан</label>
                        <Select
                    ...
                    {currentField && currentField.field_type === 'checkbox' && (
                      <p className="text-xs text-blue-600">
                        ...
                        Запись определена, если флаг установлен
                      </p>
                    )}
                    ...
                    {currentField && isSelectType(currentField) && !sf.enumId && (
                      <p className="text-xs text-amber-600">
                        Выберите значение для определения записи
                      </p>
                    )}
                  </div>
                );
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="text-sm">
                      ...
                      <p className="text-blue-600 dark:text-blue-400 text-xs mt-1">
                        Событие Schedule отправится в Facebook при совпадении любого условия
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3">
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-xs text-gray-600 dark:text-gray-300">
                    Событие Schedule (Level 3) используется для оптимизации рекламы Facebook
                    на записи на консультацию/встречу.
                  </p>
                </div>
              </div>
            </>
...
export default AmoCRMScheduledFieldModal;
