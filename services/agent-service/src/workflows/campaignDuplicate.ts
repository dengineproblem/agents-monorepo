...
async function fetchAdsetById(adsetId:string,t:string){
  const fields='id,name,status,campaign_id,daily_budget,lifetime_budget,start_time,end_time,bid_strategy,bid_amount,optimization_goal,billing_event,promoted_object,targeting,destination_type';
  const r=await withStep('fetch_adset',{adsetId,fields},()=>graph('GET',`${adsetId}`,t,{fields}));
  return r as AdsetInfo & { campaign_id: string };
}
...
async function createAdsetFromSource(
  ...
  bid.bid_strategy = 'LOWEST_COST_WITHOUT_CAP';
  if ((bid as any).bid_amount !== undefined) { delete (bid as any).bid_amount; }
}
...
// Duplicate single ad set with overriding audience include (custom_audiences)
export async function workflowDuplicateAdsetWithAudience(p:DupAdsetWithAudienceParams, t:string) {
  const srcAs = await fetchAdsetById(p.source_adset_id, t);
  const srcCamp = await fetchCampaign(String((srcAs as any).campaign_id), t);
  ...
  const budget:any = {};
  if (!isCbo) {
    const srcDaily = srcAs.daily_budget ? Number(srcAs.daily_budget) : 0;
    const desired = typeof p.daily_budget === 'number' ? Math.round(Math.max(300, Math.min(10000, p.daily_budget))) : srcDaily || 300;
    budget.daily_budget = desired;
  }

  // Bid strategy carry-over (with WA special case from existing code)
  const isWA = (srcAs.destination_type==='WHATSAPP')||(srcAs.optimization_goal==='CONVERSATIONS');
  const bs=(srcAs.bid_strategy||undefined) as (string|undefined);
  const ba=(srcAs.bid_amount!==undefined&&srcAs.bid_amount!==null&&srcAs.bid_amount!=='')?Number(srcAs.bid_amount):undefined;
  const bid:any={};
  if(bs==='LOWEST_COST_WITHOUT_CAP'){bid.bid_strategy=bs;}
  else if((bs==='LOWEST_COST_WITH_BID_CAP'||bs==='COST_CAP')&&typeof ba==='number'){bid.bid_strategy=bs;bid.bid_amount=ba;}
  else if (isWA) { bid.bid_strategy = 'LOWEST_COST_WITHOUT_CAP'; if ((bid as any).bid_amount !== undefined) { delete (bid as any).bid_amount; } }

  const newName = `${srcAs.name} ${p.name_suffix ? p.name_suffix : '— DUP LAL3 IG365'}`.slice(0, 255);
  ...
  const r=await withStep('create_adset_with_audience',{payload:createBody},()=>graph('POST',`act_${accountId}/adsets`,t,toParams(createBody)));
  const newAdsetId = r?.id; if(!newAdsetId) throw Object.assign(new Error('create_adset_failed'),{step:'create_adset_no_id'});

  // copy ads
  const ads=await fetchAds(srcAs.id,t);
  for(const ad of ads) await createAdFromSource(accountId, newAdsetId, ad, t);

  return { new_adset_id: String(newAdsetId) };
}
