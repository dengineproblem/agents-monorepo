...
export async function aiBotConfigurationsRoutes(app: FastifyInstance) {
  ...
  app.get('/whatsapp-instances', async (request, reply) => {
    ...
    try {
      ...
      // Fetch active WABA numbers for optional exposure/sync as logical instances
      ...
      // For includeWaba=true we make sure logical WABA instances exist in whatsapp_instances.
      if (includeWabaInstances && (wabaNumbers?.length || 0) > 0) {
        ...
        for (const number of wabaNumbers || []) {
          ...
          // Keep whatsapp_phone_numbers in sync for deterministic channel routing.
        ...
        if (upsertPayload.length > 0) {
          ...
          if (upsertError) {
            app.log.error({
              ...
              userId,
              syncedRows: upsertPayload.length
      ...
      // Get instance statuses from Evolution PostgreSQL for Evolution instances.
      ...
      // includeWaba=true is used by Bot Editor/Chats to include WABA.
