...
async function check() {
  const { data } = await supabase
    ...
    .select('contact_name, interest_level, engagement_trend, drop_point, hidden_objections, score, last_unanswered_message')
    .eq('user_account_id', '0f559eb0-53fa-4b6a-a51b-5d3e15e5864b')
    .order('updated_at', { ascending: false })
    .limit(10);

  console.log('=== Последние 10 анализов ===');
  data?.forEach((d, i) => {
    console.log(`\n${i+1}. ${d.contact_name || 'Без имени'}`);
    console.log(`   Interest: ${d.interest_level}, Score: ${d.score}`);
    console.log(`   Trend: ${d.engagement_trend || 'N/A'}`);
    console.log(`   Last unanswered: ${d.last_unanswered_message || 'N/A'}`);
