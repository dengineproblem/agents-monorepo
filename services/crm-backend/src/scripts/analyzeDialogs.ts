...
2. DROP_POINT (ВАЖНО - заполняй ВСЕГДА когда диалог "застрял"):
   Опиши на КАКОМ ЭТАПЕ клиент "отвалился" в ЛЮБОМ из случаев:

   a) Есть last_unanswered_message (агент написал, клиент не ответил):
   ...
   b) Клиент дал ОДНОСЛОЖНЫЙ ответ и больше не писал:
      → "Ответ 'ок' на вопрос о встрече - формальное согласие без действий"
      → "Короткий ответ на вопрос о бюджете - избегает темы"

   c) Последнее сообщение клиента было ДАВНО (12+ часов без продолжения):
      → "Последний вопрос о [тема] - клиент пропал после"

   d) Диалог НЕ завершён успешно (нет записи/сделки) и активности нет:
      → Опиши последний значимый момент где диалог застрял

   - Если диалог активный/завершён успешно → null
...
`;
    ...
    // Now with date filtering to only get NEW leads from the specified period
    const newLeadMessages = await getNewLeads(instanceName, minIncoming, existingPhonesWithSuffix, startDate, endDate);
