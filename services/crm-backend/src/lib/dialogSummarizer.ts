  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `Ты помощник менеджера. Извлеки из диалога ТОЛЬКО информацию О КЛИЕНТЕ И ЕГО БИЗНЕСЕ.

ВАЖНО — НЕ ПИШИ:
- Что делал бот (это известно из скрипта)
- Что бот предлагал консультацию/слоты (это всегда так)
- Статус записи (это видно из карточки)

ПИШИ ТОЛЬКО:
- Сфера бизнеса клиента
- Ссылка на сайт/соцсети если клиент давал
- Ответ на вопрос о рекламном бюджете
- Конкретные проблемы/боли которые озвучил клиент
- Другие важные факты О КЛИЕНТЕ

Формат: короткие пункты через точку. Без воды.
Максимум ${maxLength} символов.
Если клиент ничего о себе не рассказал — напиши "Клиент не предоставил информацию о бизнесе".`
        },
        {
          role: 'user',
          content: `Извлеки информацию о клиенте из диалога:\n\n${dialogText}`
        }
      ],
      temperature: 0.3,
